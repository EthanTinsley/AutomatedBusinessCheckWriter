...
class DataBaseHelper:
    ...
    def on_create(self):
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS Bank (
            bank_id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT,
        ...
        c.execute("""CREATE TABLE IF NOT EXISTS Signature(
            signature_id INTEGER NOT NULL PRIMARY KEY AUTO_INCREMENT,
    ...
    def upload_company(self, company):
        ...
        c.execute("INSERT INTO Company VALUES(  %(company_id)s , %(company_name)s ,  %(address)s , %(city)s , %(state)s , %(zip)s )",
    ...
    def get_company_id(self, company):
        ...
        c.execute("SELECT company_id FROM Company WHERE company_name = %(name)s ; " , {'name': company_name})
    ...
    def get_company(self, account_code):
        ...
        c.execute("""SELECT company_name, address, city, state, zip FROM Company WHERE company_id IN
                    (SELECT company_id FROM Account WHERE account_code = %(account_code)s );""", 
                    {'account_code': code})
    ...
    def remove_company(self, id):
        ...
        # sql statement
        c.execute("""DELETE FROM Company WHERE company_id = %(id)s ;""" , {'id': id})
    ...
    def upload_bank(self, Bank):
        ...
        c.execute("INSERT INTO Bank VALUES(%(bank_id)s, %(bank_name)s , %(address)s , %(city)s , %(state)s , %(zip)s ) ;" , 
    ...
    def get_bank_id(self, bank):
        ...
        c.execute("SELECT bank_id FROM Bank WHERE bank_name = %(name)s " , {'name': bank_name})
    ...
    def get_bank(self, account_code):
        ...
        c.execute("""SELECT bank_name, address, city, state, zip FROM Bank WHERE bank_id IN
                    (SELECT bank_id FROM Account WHERE account_code = %(account_code)s );""", 
                    {'account_code': code})
    ...
    def remove_bank(self, bank_id):
        conn = self.get_connection(buffered=True)
        c = conn.cursor()

        # SQL delete query
        c.execute("""DELETE FROM Bank WHERE bank_id = %(id)s ;""", {'id': bank_id})
    ...
    def upload_account(self, account, company, bank, signature):
        ...
        c.execute("INSERT INTO Account VALUES ( %(account_id)s , %(account_code)s , %(account_num)s , %(routing_num)s , %(company_id)s , %(bank_id)s, %(signature_id)s );", 
    ...
    def get_account(self, account_code):
        ...
        # pass SQL query to the database in order to return the appropriate informaiton 
        c.execute("SELECT account_code, account_num, routing_num, account_id FROM Account WHERE account_code = %(account_code)s ;",
         {'account_code': code })
    ...
    def remove_account(self, account_code):
        ...
        c.execute("DELETE FROM Account WHERE account_code = %(code)s ;", {'code': account_code})
    ...
    def upload_signature(self, signature):
        ...
        c.execute("INSERT INTO Signature VALUES( %(id)s , %(name)s, %(path)s ) ;", {'id': None, 'name': signature.name, 'path': signature.path})
    ...
    def get_signature(self, account_code):
        ...
        c.execute("SELECT signature_name, signature_imgpath, signature_id FROM Signature WHERE signature_id IN (SELECT signature_id FROM Account WHERE account_code = %(code)s ) ;", 
        {'code': account_code})
    ...
    def remove_signature(self, signature_id):
        ...
        c.execute("""DELETE FROM Signature WHERE signature_id = %(id)s ;""" , {'id': signature_id})
    ...
    def upload_check(self, Check, printed=False):
        ...
        c.execute("INSERT INTO CheckTemplate VALUES( %(check_id)s, %(printed)s, %(upload_date)s, %(check_num)s, %(check_date)s, %(invoice_num)s, %(invoice_date)s, %(payee)s, %(amount)s, %(account_code)s )" ,
    ...
    def get_unprinted_checks(self):
        ...
        c = conn.cursor(buffered=True) # cursor used ot pass and retrieve information from queries 
    ...
    def remove_check(self,check_id):
        ...
        c.execute("DELETE FROM CheckTemplate WHERE check_id = %(id)s", {'id': check_id})
    ...
    def duplicate_check_test(self, check):
        ...
        # sql query 
        c.execute("SELECT COUNT(*) FROM CheckTemplate WHERE check_num = %(num)s AND amount = %(amount)s ;", {'num': check.check_num, 'amount': check.amount})
    ...
    def update_check_printed(self, check, printed=True):
        ...
        c.execute("UPDATE CheckTemplate SET printed = %(print)s WHERE check_num = %(num)s ;", {'print':printed , 'num': check.check_num})
